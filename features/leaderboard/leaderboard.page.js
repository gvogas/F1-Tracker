var LeaderboardPageModel = (function () {
  ...
  function loadMeetings(force) {
    ...
    OpenF1API.meetings({ year: state.year })
      ...
      .fail(function () {
        showMsg("Failed to load meetings. Check your connection.");
      });
  }
  ...
  function loadSessionsForMeeting(meetingKey, force) {
    ...
    OpenF1API.sessions({ meeting_key: meetingKey })
      ...
      .fail(function () {
        showMsg("Failed to load sessions. Check your connection.");
      });
  }
  ...
  function loadResultsForSession(sessionKey) {
    ...
    .done(function (normRes, resultRes) {
      // normRes might be [data,status,xhr] if resolved via $.ajax
    ...
    .fail(function () {
      showMsg("Failed to load results. Check your connection.");
      $("#driversBody").empty();
    });
  }
  ...
  function render(results, byNum) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      var status   = r.dsq ? "DSQ" : (r.dns ? "DNS" : (r.dnf ? "DNF" : "Finished"));
